...
postgres_exporter_id=$(image_id sentinelops-postgres-exporter:0.1.0-local)
...
umask 077
{
  ...
  printf '  postgres-exporter: { image: "%s", pull_policy: never }\n' "$postgres_exporter_id"
} > "$temporary"
chmod 600 "$temporary"
mv "$temporary" "$OUTPUT"
...
printf 'image_lock=%s\napi=%s\nworker=%s\nweb=%s\ndemo=%s\nalloy=%s\ncaddy=%s\npostgres_exporter=%s\n' \
	"$OUTPUT" "$api_id" "$worker_id" "$web_id" "$demo_id" "$alloy_id" "$caddy_id" "$postgres_exporter_id"
