...
wait_ready() {
  ...
  until curl -fsS "$url" >/dev/null 2>&1; do
    attempts=$((attempts + 1))
    [ "$attempts" -lt 30 ] || { echo "Timeout aguardando $url" >&2; return 1; }
    sleep 3
  done
}
...
wait_ready http://localhost:4040/ready
echo "Control Plane, Web, demo e backends de telemetria responderam."
